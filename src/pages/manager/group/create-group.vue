  ...
  <div class="">
    ...
    <div class="fixed top-0 right-0 w-1/5 h-screen bg-white z-50 p-5">
      ...
      <div class="flex flex-col gap-5">
        <select class="w-full focus:outline-none pr-12 bg-gray-100 px-4 py-2 rounded" v-model="formData.direction_id">
          <option v-for="item, index in getData.direction" :key="index" :value="item.id">{{ item.name }}</option>
        </select>
        <select class="w-full focus:outline-none pr-12 bg-gray-100 px-4 py-2 rounded" v-model="formData.day">
          <option v-for="item, index in getData.day" :key="index" :value="item.id">{{ item.name }}</option>
        </select>
        <select class="w-full focus:outline-none pr-12 bg-gray-100 px-4 py-2 rounded" v-model="formData.time">
          <option v-for="item, index in getData.time" :key="index" :value="item.id">{{ item.name }}</option>
        </select>
        <select class="w-full focus:outline-none pr-12 bg-gray-100 px-4 py-2 rounded" v-model="formData.room_id">
      ...
      <button class="bg-[#166199] rounded py-2.5 px-5 flex gap-1 text-white mt-4" @click="createGroup">
